  ...
  for path, dname, fnames  in os.walk(inputDir):
    ...
    for fname in fnames:
      ...
      if descExt in fname.lower():
        remain = path.split(os.path.sep)[:-3]
        rname = path.split(os.path.sep)[-3]
        cname = path.split(os.path.sep)[-2]
        sname = path.split(os.path.sep)[-1]
        if cname not in dataDetails:
          dataDetails[cname]={}
        fDict = json.load(open(os.path.join(os.path.sep.join(remain), rname, cname, sname, fname),'r'))
